...
const d = new Date();
var log_file = fs.createWriteStream(path_1.default.join(__dirname, 'logs', `${d.getDate()}-${d.getMonth() + 1}-${d.getFullYear()}_categories.log`), { flags: "w" });
...
(async () => {
    ...
    try {
        ...
        const changeList = new ChangeList_1.default(DOWNLOAD_DIR, "sh_category_creations", "sh_category_updates");
        shopify.mutator.setSavePaths(DOWNLOAD_DIR, changeList.createFileName, changeList.updateFileName);
        const compare = new Comparison_1.default(changeList);
